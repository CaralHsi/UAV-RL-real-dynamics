...
class World(object):
    ...
    def step(self):
        ...
        p_force_agent = [None] * len(self.entities)
        p_force_environment = [None] * len(self.entities)
        # apply agent physical controls
        p_force_agent = self.apply_action_force(p_force_agent)
    ...
    def apply_environment_force(self, p_force=None):
    ...
    def integrate_state(self, p_force_agent, p_force_environment):
        for i, entity in enumerate(self.entities):
            ...
            '''if p_force_environment[i] is not None:
                entity.state.p_vel += (p_force_environment[i] / entity.mass) * self.dt'''
            '''constraint_force = np.sum(np.square(agent.state.p_vel) + np.square(agent.state.theta))
            entity.state.p_vel * p_force_agent[i][0]
